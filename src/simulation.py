...
def main():
    ...
    grids = [empty, G_1W1, G_1W2, G_AE1, G_AE2, G_AE3,
             G_AE4, G_EG1, G_MG1, G_MG2, G_SF1, G_SF2]
...
def many_simulations(ai_service, grids, depth, timeout):
    ...
    mm_results = []
    print(
        f"Starting multiple iterative deepening with max depth {depth} and timeout {timeout}s")
    start_time = time.time()
    for grid in grids:
        res = ai_service.calculate_next_move_id_minimax(
            grid, 1, timeout, depth)
...
def deep_simulation(ai_service, grids, depth, timeout):
    ...
    print(
        f"starting Early-game iterative deepening with max depth {depth} and timeout {timeout}s")
    eg_id = ai_service.calculate_next_move_id_minimax(
        grids[0], 1, timeout, depth)
    ...
    print("")
    print(
        f"starting Mid-game iterative deepening with max depth {depth} and timeout {timeout}s")
    mg_id = ai_service.calculate_next_move_id_minimax(
        grids[1], 1, timeout, depth + 2)
